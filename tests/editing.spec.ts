...
test('rich content formatting stays within the selected node', async ({
  page,
}) => {
  ...
  await editor.fill('Understand the system')
  await editor.press('Meta+a')
  await page.getByRole('button', { name: 'Bold', exact: true }).click()
  await expect(editor.locator('strong')).toHaveText('Understand the system')
  await page.getByRole('button', { name: 'Italic', exact: true }).click()
  await expect(editor.locator('em')).toHaveText('Understand the system')
  await page.getByLabel('Font size').selectOption('20px')
  await expect(editor.locator('span')).toHaveAttribute(
    'style',
    /font-size: 20px/,
  )
